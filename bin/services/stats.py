...
def action(gwss, service, action, client, data):
	...
	if action == "add_client":
		gwss.logger.debug("%s:%s:%s:%s" % (service.name, action, id(client), service.track[id(client)]['ip']))
		gwss.logger.debug("%s:%s:%s:%s" % (service.name, action, id(client), service.stat2))
		geo = geolite2.lookup(service.track[id(client)]['ip'])
		country = geo.country.lower()
		gwss.logger.debug("%s:%s:%s:%s" % (service.name, action, id(client), country))
		if not country:
			country = "N/A"
		service.track[id(client)]['country'] = country
		try:
			service.stat2[country] += 1
		except:
			service.stat2[country] = 1
			pass
	...
	if action == "del_client":
		# have no more info !!!
		country = service.track[id(client)]['country']
		try:
			service.stat2[country] -= 1
		except:
			pass
...
def stat2(gwss,service,client):
	...
	worldmap_chart.render_to_file("/var/www/public_html/static/img/stat2.svg")
