...
def api(gwss, service, action, data):
	...
	"""
	print("%s:api:%s:%s" % (service.name, action, data))
	msg = "error:unhandled event"
	if action == "set":
		data = {"service": "echo", "action": "set", "data": data}
		msg = json.dumps(data)
		print("%s:api:set:send_all:%s" % (service.name, msg))
		service.send_all(msg)
	if action == "get":
		ident = data["id"]
		value = data["value"]
		print("%s:api:get:send_all:%s" % (service.name, msg))
